<template>
	<div :class="`player --${direction}`">
		...
		<div :class="[`background --top-row --${direction} --${side}`, {'--alive': player.state.health, '--player-focused': focused }]"></div>
		<div :class="[`background --bottom-row --${direction} --${side}`, { '--alive': player.state.health, '--player-focused': focused, }]"></div>
