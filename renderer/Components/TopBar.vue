			...
			<template v-for="(team, direction) in { left, right }">
				<div :class="`team-color-patch --${direction}`">
					<div :class="`team --${direction}`">
						<div v-if="team.name"></div>
							<img class="team-img" :src="require(`../../img/teams/${team.name}.svg`)">
					</div>
					<div :class="`inner --${directionalSides[Number(direction === 'right')]}`"></div>
				</div>

				<div :class="`logo-wrapper --${direction}`">
					<div :class="`logo-t --${direction}`">	
						<div v-if="team.name"></div>
						<!-- <img :src="require(`../../img/logos/${team.name}.svg`)" /> -->

						<!-- {{team.name}} -->
					</div>

					<!-- <div
						:class="`logo ${flagStyle(team.name)} --${directionalSides[Number(direction === 'right')]}`"
						:style="{ 'background-image': team.name && `url('../../img/logos/${team.name.toLowerCase()}.svg')` }"
						>
					</div> -->
					<div
						class="team-img"
				...
				<div v-if="seriesData.bestOf !== 1" :class="`map-wins --${direction}`">
					...
					<div :class="[`bof --${direction}`]">
						<div :class="`inner --${direction}`">Best of 3</div>
					</div>
				</div>
